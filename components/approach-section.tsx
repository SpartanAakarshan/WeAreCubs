            ...
            {pillars.map((pillar, index) => {
              ...
              return (
                <div key={index} className="relative group">
                  <div className="bg-black text-white rounded-lg p-8 space-y-4 h-full border-4 border-white/20 hover:border-primary transition-all hover:scale-105">
                    {/* <div className="w-16 h-16 bg-primary rounded-lg flex items-center justify-center">
                      <Icon className="w-8 h-8 text-primary-foreground" />
                    </div> */}
                    <h3 className="text-3xl font-black text-white text-center">{pillar.title}</h3>
                    <div className="overflow-hidden max-h-0 opacity-0 translate-y-2 group-hover:max-h-40 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 ease-out">
